...
class IntelligenceSource(BaseModel):
    ...
    type: str = Field(
        ...,
        description="Type of source data, e.g., 'review', 'campaign', 'ad_copy', 'metric'",
    )
    ...
    freshness: float = Field(
        ..., ge=0.0, le=1.0, description="Freshness score from 0.0 to 1.0"
    )
    reliability: float = Field(
        ..., ge=0.0, le=1.0, description="Reliability score from 0.0 to 1.0"
    )
    raw_data: Dict[str, Any] = Field(
        default_factory=dict, description="Raw unstructured data from the source"
    )
...
class NeuroInsight(BaseModel):
    ...
    category: InsightCategory = Field(
        ...,
        description="Category of the insight: Priority, Risk, Opportunity, or Trend",
    )
    ...
    actionable_items: List[str] = Field(
        ...
        description="List of suggested actions based on this insight",
    )
