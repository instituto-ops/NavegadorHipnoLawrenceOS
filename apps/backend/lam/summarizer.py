...
class SummarizerOutput(BaseModel):
    category: InsightCategory = Field(
        ..., description="Category: Priority, Risk, Opportunity, or Trend"
    )
...
def create_summarizer_chain():
    ...
    prompt = ChatPromptTemplate.from_messages(
        [
            (
                "system",
                """You are the NeuroEngine Summarizer. Your goal is to convert raw execution logs, extracted text, and metrics into a highly structured NeuroInsight.
...
async def process_execution_results(
    results: List[Any], task_description: str
) -> List[NeuroInsight]:
    """
...
if __name__ == "__main__":
    ...
    async def test():
        ...
        insights = await process_execution_results(
            mock_results, "Analyze recent reviews"
        )
        for i in insights:
