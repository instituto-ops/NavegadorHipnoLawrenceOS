...
async def run_agent(task: str, websocket: WebSocket):
    ...
    llm = ChatGroq(model="llama3-70b-8192", temperature=0.0) # type: ignore
    ...
    try:
        ...
        agent = Agent(task=task, llm=llm, browser=browser) # type: ignore
        ...
        async for state in agent.run_step_by_step(): # type: ignore
            if state is None:
                continue
