...
async def run_jules_command(request: JulesRequest, websocket: WebSocket):
    ...
    if not command.startswith("jules"):
        await websocket.send_json(
            JulesResponse(
                type="jules_error", message="Error: Command must start with 'jules'"
            ).model_dump()
        )
        return
    ...
    try:
        ...
        await websocket.send_json(
            JulesResponse(
                type="jules_error", message=f"Error parsing command: {e}"
            ).model_dump()
        )
        return
    ...
    if not args or args[0] != "jules":
        await websocket.send_json(
            JulesResponse(
                type="jules_error", message="Error: Invalid command format"
            ).model_dump()
        )
        return
    ...
    try:
        ...
        async def read_stream(
            stream: asyncio.StreamReader, ws: WebSocket, stream_type: str
        ):
            while True:
        ...
        await process.wait()
        await websocket.send_json(
            JulesResponse(type="jules_done", exit_code=process.returncode).model_dump()
        )
        ...
        await websocket.send_json(
            JulesResponse(
                type="jules_error", message="Error: jules executable not found"
            ).model_dump()
        )
        await websocket.send_json(
            JulesResponse(type="jules_done", exit_code=127).model_dump()
        )
    except Exception as e:
        await websocket.send_json(
            JulesResponse(type="jules_error", message=str(e)).model_dump()
        )
