    ...
    try:
        while True:
            ...
            if msg.type == "lam" and msg.task:
                ...
                response_config: Any = {
                    "configurable": {"thread_id": response_thread_id}
                }
                ...
                if msg.action == "approve":
                    ...
                    await orchestrator.graph.aupdate_state(
                        response_config, {"hitl_approved": True}
                    )
                    current_task = asyncio.create_task(
                        process_lam_stream(None, response_config, response_thread_id)
                    )
                    ...
                    await orchestrator.graph.aupdate_state(
                        response_config, {"plan": msg.plan, "hitl_approved": True}
                    )
                    current_task = asyncio.create_task(
                        process_lam_stream(None, response_config, response_thread_id)
                    )
                    ...
                    await orchestrator.graph.aupdate_state(
                        response_config, {"hitl_approved": False}
                    )
                    current_task = asyncio.create_task(
                        process_lam_stream(None, response_config, response_thread_id)
                    )
