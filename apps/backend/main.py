            ...
            if msg.type == "lam" and msg.task:
                ...
                if msg.action == "approve":
                    ...
                    await orchestrator.graph.aupdate_state(
                        config_hitl, {"hitl_approved": True}
                    )
                    current_task = asyncio.create_task(
                        process_lam_stream(None, config_hitl, thread_id)
                    )
                    ...
                    await orchestrator.graph.aupdate_state(
                        config_hitl, {"plan": msg.plan, "hitl_approved": True}
                    )
                    current_task = asyncio.create_task(
                        process_lam_stream(None, config_hitl, thread_id)
                    )
                    ...
                    await orchestrator.graph.aupdate_state(
                        config_hitl, {"hitl_approved": False}
                    )
                    current_task = asyncio.create_task(
                        process_lam_stream(None, config_hitl, thread_id)
                    )
