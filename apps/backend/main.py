...
async def websocket_endpoint(websocket: WebSocket):
    ...
    async def process_lam_stream(initial_state, run_config, thread_id):
        try:
            async for event in orchestrator.graph.astream(initial_state, run_config):
            ...
            # Check for HITL
            snapshot = await orchestrator.graph.aget_state(run_config)
    ...
    try:
        while True:
            ...
            if msg.type == "lam" and msg.task:
                ...
                current_task = asyncio.create_task(process_lam_stream(initial_state, run_config, thread_id))
                ...
                if msg.action == "approve":
                    ...
                    await orchestrator.graph.aupdate_state(run_config, {"hitl_approved": True})
                    current_task = asyncio.create_task(process_lam_stream(None, run_config, thread_id))
                elif msg.action == "edit":
                    await websocket.send_text(json.dumps({"type": "log", "message": "Plan edited. Resuming..."}))
                    await orchestrator.graph.aupdate_state(run_config, {"plan": msg.plan, "hitl_approved": True})
                    current_task = asyncio.create_task(process_lam_stream(None, run_config, thread_id))
                elif msg.action in ["reject", "cancel"]:
                    await websocket.send_text(json.dumps({"type": "log", "message": "Task aborted."}))
                    await orchestrator.graph.aupdate_state(run_config, {"hitl_approved": False})
                    current_task = asyncio.create_task(process_lam_stream(None, run_config, thread_id))
