...
async def websocket_endpoint(websocket: WebSocket):
    ...
    async def process_lam_stream(initial_state, thread_config_map, thread_id):
        try:
            async for event in orchestrator.graph.astream(initial_state, thread_config_map):
            ...
            # Check for HITL
            snapshot = await orchestrator.graph.aget_state(thread_config_map)
    ...
    try:
        while True:
            ...
            if msg.type == "lam" and msg.task:
                ...
                thread_config_map = {"configurable": {"thread_id": thread_id}}
                initial_state: Any = {
                    "task": msg.task,
                    "plan": {},
                    "execution_results": [],
                ...
                current_task = asyncio.create_task(process_lam_stream(initial_state, thread_config_map, thread_id))
                ...
                if msg.action == "approve":
                    ...
                    await orchestrator.graph.aupdate_state(thread_config_map, {"hitl_approved": True})
                    current_task = asyncio.create_task(process_lam_stream(None, thread_config_map, thread_id))
                elif msg.action == "edit":
                    await websocket.send_text(json.dumps({"type": "log", "message": "Plan edited. Resuming..."}))
                    await orchestrator.graph.aupdate_state(thread_config_map, {"plan": msg.plan, "hitl_approved": True})
                    current_task = asyncio.create_task(process_lam_stream(None, thread_config_map, thread_id))
                elif msg.action in ["reject", "cancel"]:
                    await websocket.send_text(json.dumps({"type": "log", "message": "Task aborted."}))
                    await orchestrator.graph.aupdate_state(thread_config_map, {"hitl_approved": False})
                    current_task = asyncio.create_task(process_lam_stream(None, thread_config_map, thread_id))
