...
# Use environment variable for allowed origins to prevent unauthorized access
    ...
    try:
        while True:
            ...
            if msg.type == "lam" and msg.task:
                ...
                config_hitl: Any = {"configurable": {"thread_id": thread_id}}

                if msg.action == "approve":
                    await websocket.send_text(json.dumps({"type": "log", "message": "Plan approved. Resuming..."}))
                    await orchestrator.graph.aupdate_state(config_hitl, {"hitl_approved": True})
                    current_task = asyncio.create_task(process_lam_stream(None, config_hitl, thread_id))
                elif msg.action == "edit":
                    await websocket.send_text(json.dumps({"type": "log", "message": "Plan edited. Resuming..."}))
                    await orchestrator.graph.aupdate_state(config_hitl, {"plan": msg.plan, "hitl_approved": True})
                    current_task = asyncio.create_task(process_lam_stream(None, config_hitl, thread_id))
                elif msg.action in ["reject", "cancel"]:
                    await websocket.send_text(json.dumps({"type": "log", "message": "Task aborted."}))
                    await orchestrator.graph.aupdate_state(config_hitl, {"hitl_approved": False})
                    current_task = asyncio.create_task(process_lam_stream(None, config_hitl, thread_id))
