    ...
    try:
        while True:
            ...
            if msg.type == "lam" and msg.task:
                ...
                response_thread_id = msg.thread_id
                response_config: Any = {"configurable": {"thread_id": response_thread_id}}

                if msg.action == "approve":
                    await websocket.send_text(json.dumps({"type": "log", "message": "Plan approved. Resuming..."}))
                    await orchestrator.graph.aupdate_state(response_config, {"hitl_approved": True})
                    current_task = asyncio.create_task(process_lam_stream(None, response_config, response_thread_id))
                elif msg.action == "edit":
                    await websocket.send_text(json.dumps({"type": "log", "message": "Plan edited. Resuming..."}))
                    await orchestrator.graph.aupdate_state(response_config, {"plan": msg.plan, "hitl_approved": True})
                    current_task = asyncio.create_task(process_lam_stream(None, response_config, response_thread_id))
                elif msg.action in ["reject", "cancel"]:
                    await websocket.send_text(json.dumps({"type": "log", "message": "Task aborted."}))
                    await orchestrator.graph.aupdate_state(response_config, {"hitl_approved": False})
                    current_task = asyncio.create_task(process_lam_stream(None, response_config, response_thread_id))
