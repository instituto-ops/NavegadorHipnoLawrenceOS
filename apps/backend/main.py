...
async def health_check():
    return {"status": "ok", "service": "Navegador Hipnótico Lawrence OS (NeuroOS)"}
...
async def websocket_endpoint(websocket: WebSocket):
    ...
    async def process_lam_stream(initial_state, ws_config, thread_id):
        try:
            async for event in orchestrator.graph.astream(initial_state, ws_config):
            ...
            # Check for HITL
            snapshot = await orchestrator.graph.aget_state(ws_config)
    ...
    try:
        while True:
            ...
            if msg.type == "lam" and msg.task:
                ...
                ws_config: Any = {"configurable": {"thread_id": thread_id}}
                initial_state: Any = {
                    "task": msg.task,
                    "plan": {},
                    "execution_results": [],
                ...
                current_task = asyncio.create_task(process_lam_stream(initial_state, ws_config, thread_id))
                ...
                ws_config2: Any = {"configurable": {"thread_id": thread_id}}

                if msg.action == "approve":
                    await websocket.send_text(json.dumps({"type": "log", "message": "Plan approved. Resuming..."}))
                    await orchestrator.graph.aupdate_state(ws_config2, {"hitl_approved": True})
                    current_task = asyncio.create_task(process_lam_stream(None, ws_config2, thread_id))
                elif msg.action == "edit":
                    await websocket.send_text(json.dumps({"type": "log", "message": "Plan edited. Resuming..."}))
                    await orchestrator.graph.aupdate_state(ws_config2, {"plan": msg.plan, "hitl_approved": True})
                    current_task = asyncio.create_task(process_lam_stream(None, ws_config2, thread_id))
                elif msg.action in ["reject", "cancel"]:
                    await websocket.send_text(json.dumps({"type": "log", "message": "Task aborted."}))
                    await orchestrator.graph.aupdate_state(ws_config2, {"hitl_approved": False})
                    current_task = asyncio.create_task(process_lam_stream(None, ws_config2, thread_id))
