...
const ScoreGauge = ({ score, title }: { score: number; title: string }) => {
    ...
    const COLORS = score >= 90 ? ['#2EED8F', '#1A1A1A'] : score >= 50 ? ['#FACC15', '#1A1A1A'] : ['#EF4444', '#1A1A1A'];
...
export const SeoIntelligence: React.FC = () => {
    ...
    const runAnalysis = async () => {
        ...
        setError(null);
        try {
            const response = await fetch(`http://localhost:8000/api/analytics/pagespeed?url=${encodeURIComponent(url)}`);
                ...
                <div className="lg:col-span-2 bg-[#111111] border border-gray-800/60 rounded-xl p-6 flex flex-col">
                    ...
                    <div className="space-y-4 flex-1">
                        <div className="flex items-center justify-between p-4 bg-[#1A1A1A] rounded-lg border border-gray-800">
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    ...
                                    <p className="text-[11px] text-gray-500">Configurados e dentro do tamanho ideal.</p>
                                </div>
                            </div>
                        ...
                        <div className="flex items-center justify-between p-4 bg-[#1A1A1A] rounded-lg border border-gray-800 opacity-60">
                            ...
                            <button className="text-[10px] font-bold text-[#2EED8F] underline">Corrigir Agora</button>
                        </div>
                    </div>
                    ...
                    <div className="mt-6 p-4 bg-blue-500/5 border border-blue-500/20 rounded-lg flex gap-3">
                        ...
                        <p className="text-[11px] text-blue-200/80 leading-relaxed">
                            <strong>Insight da IA:</strong> Seu site tem ótima pontuação, mas você pode melhorar a densidade da palavra-chave "hipnose clínica" nos primeiros 10% do conteúdo para ganhar mais tração regional.
                        </p>
                    </div>
                </div>
                    ...
                    <div className="space-y-3">
                        <div className="p-3 bg-gray-900/50 rounded-lg border border-gray-800">
                            <div className="flex items-center justify-between mb-1">
                                <span className="text-[9px] font-bold text-purple-400 uppercase tracking-tighter">Pergunta Popular</span>
                                <span className="text-[9px] text-gray-600">Alta Vol.</span>
                            </div>
                            <p className="text-xs text-white">Como a hipnose pode ajudar no tratamento do autismo em adultos?</p>
                        </div>

                        <div className="p-3 bg-gray-900/50 rounded-lg border border-gray-800">
                            <div className="flex items-center justify-between mb-1">
                                <span className="text-[9px] font-bold text-blue-400 uppercase tracking-tighter">Comparação</span>
                                <span className="text-[9px] text-gray-600">Tendência Jan/26</span>
                            </div>
                            <p className="text-xs text-white">Hipnose vs TCC: Qual a melhor para ansiedade generalizada?</p>
                        </div>

                        <div className="p-3 bg-gray-900/50 rounded-lg border border-gray-800">
                            <div className="flex items-center justify-between mb-1">
                                <span className="text-[9px] font-bold text-green-400 uppercase tracking-tighter">Preposição</span>
